...
exports.createPrime = async (req, res, next) => {
  let options = {
  ...
  const primeDetail = await instance.orders.create(options);

  if (!primeDetail) return res.status(500).send("Some error occured");

  res.status(200).json({ success: true, primeDetail });
};
...
exports.paymentCallback = async (req, res, next) => {
  ...
  try {
    ...
    console.log(user);
    user.primeMember = true;
    user.joinedPrimeDate = Date.now();
    user.remainingPrimeAmount = 1500;
    user.primePaymentInfo = paymentInfo;
    await user.save();
  } catch (err) {
    console.log(err);
  ...
  res.json({
    message: "Congratulations, You are our prime member now " ,
